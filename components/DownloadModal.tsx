...
export const DownloadModal: React.FC<DownloadModalProps> = ({ isOpen, onClose, onConfirm, initialStartWeek, initialEndWeek }) => {
    ...
    const WeekDropdown: React.FC<{ id: string, value: number, onChange: (val: number) => void, label: string }> = ({ id, value, onChange, label }) => (
         <select
            ...
            className="w-24 bg-slate-900 border border-slate-600 rounded-lg px-2 py-1 text-slate-200 text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
            aria-label={label}
        >
    ...
    const modalContent = (
        ...
        >
            ...
            >
                <h2 className="text-xl font-bold text-slate-200 mb-4">Download Tasks</h2>
                <p className="text-slate-400 mb-6">Select the range of weeks you want to export to a CSV file.</p>
                ...
                <div className="flex justify-end gap-4">
                    ...
                    <button
                        ...
                        autoFocus
                        className="px-4 py-2 rounded-md font-semibold text-sm bg-green-600 text-white hover:bg-green-700 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500"
                    >
                        Download
                    </button>
                </div>
