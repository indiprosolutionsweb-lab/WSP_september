...
export const DayColumn: React.FC<DayColumnProps> = ({ day, tasks, onAddTask, onUpdateTask, onDeleteTask, canEditTasks, canAddTask }) => {
    ...
    return (
        <div className="bg-slate-800 rounded-xl p-1.5 flex flex-col h-full min-h-[300px] border border-slate-700/50">
            ...
            {canAddTask && (
                <div className="mt-2 flex items-center space-x-2 pt-2 border-t border-slate-700">
                    <input
                        type="text"
                        value={newTaskText}
                        ...
                        disabled={isAdding}
                        className="min-w-0 flex-grow bg-slate-700 border border-slate-600 rounded-lg px-2 py-1.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 disabled:cursor-wait"
                    />
                    <button
