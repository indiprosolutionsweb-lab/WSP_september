...
export const Header: React.FC<HeaderProps> = ({ currentUser, viewingUser, companies, viewableUsers, onLogout, onSelectViewUser, currentView, onSetCurrentView, canManageUsers, canAccessPersonalViews }) => {
    ...
    const NavButton: React.FC<{ view: ViewType; label: string; colorClass?: string }> = ({ view, label, colorClass = 'blue' }) => (
        <button onClick={() => onSetCurrentView(view)} className={`px-3 py-1.5 rounded-md font-semibold text-sm transition-colors ${currentView === view ? `bg-${colorClass}-600 text-white shadow-md` : 'bg-sky-100 text-slate-700 hover:bg-sky-200'}`}>{label}</button>
    );

    return (
        <header className="mb-4 sticky top-4 z-20">
             <div className="w-full max-w-screen-2xl mx-auto flex flex-col xl:flex-row items-center justify-between gap-4 text-sm bg-white/80 backdrop-blur-sm p-2 rounded-lg border border-slate-200/80 shadow-sm">
                <div className="flex items-center gap-2.5 shrink-0">
                    <div className="flex flex-col items-center justify-center">
                        ...
                        <span className="text-[9px] text-slate-500 font-light tracking-wider -mt-0.5">indipro</span>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-end gap-3 flex-wrap">
                    <span className="font-semibold text-slate-700 hidden sm:inline">Welcome, {currentUser.name}</span>
                    <button onClick={() => onSetCurrentView('calendar')} className={`p-1.5 rounded-full transition-colors ${currentView === 'calendar' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-sky-200'}`} title="Open year calendar"><CalendarIcon /></button>
                    <button onClick={toggleFullscreen} className="p-1.5 rounded-full text-slate-500 hover:bg-sky-200" title={isFullscreen ? "Exit fullscreen" : "Enter fullscreen"}>{isFullscreen ? <CollapseIcon /> : <ExpandIcon />}</button>
                    ...
                    {isSuperAdmin ? (
                         <>
                            <div className="flex items-center gap-2">
                                <label htmlFor="company-filter" className="font-semibold text-slate-700">Co:</label>
                                <select id="company-filter" value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)} className="bg-slate-100 border border-slate-300 rounded-md px-2 py-1 text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <div className="flex items-center gap-2">
                                <label htmlFor="view-selector" className="font-semibold text-slate-700">View:</label>
                                <select id="view-selector" value={viewingUser?.id || ''} onChange={(e) => onSelectViewUser(e.target.value)} className="bg-slate-100 border border-slate-300 rounded-md px-2 py-1 text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={filteredUsers.length === 0}>
                        ...
                        <div className="flex items-center gap-2">
                            <label htmlFor="view-selector" className="font-semibold text-slate-700">Viewing:</label>
                             <select id="view-selector" value={viewingUser?.id || ''} onChange={(e) => onSelectViewUser(e.target.value)} className="bg-slate-100 border border-slate-300 rounded-md px-2 py-1 text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
