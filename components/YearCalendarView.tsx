...
export const YearCalendarView: React.FC<YearCalendarViewProps> = ({ financialYearStartDate, financialYearString }) => {
    ...
    return (
        <div className="bg-sky-100/60 rounded-xl p-6 md:p-8 mt-4">
            <h2 className="text-3xl font-bold text-slate-800 mb-2 text-center">Year Calendar</h2>
            <p className="text-center text-slate-600 mb-6">{`Financial Year: ${financialYearString}`}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 max-h-[70vh] overflow-y-auto pr-2">
                {weeks.map(({ week, startDate, endDate }) => (
                    <div key={week} className="bg-white p-4 rounded-lg shadow-md border-l-4 border-slate-200 hover:border-blue-500 transition-colors">
                        <p className="text-slate-700 text-sm">
                            <span className="font-bold text-blue-600">Week {week}:</span> {formatDate(startDate)} to {formatDate(endDate)}
                        </p>
                    </div>
                ))}
            </div>
