...
export const FocusView: React.FC<{ note: FocusNote | null; onSave: (note: { focus_text: string | null; pointers_text: string | null; }) => void; }> = ({ note, onSave }) => {
    ...
    const StatusSelector: React.FC<{ status: FocusItemStatus; onSelect: (status: FocusItemStatus) => void; }> = ({ status, onSelect }) => {
        ...
        const colors: { [key in FocusItemStatus]: string } = { 'red': 'bg-red-500 hover:bg-red-400', 'yellow': 'bg-yellow-400 hover:bg-yellow-300', 'green': 'bg-green-500 hover:bg-green-400', 'none': 'bg-slate-500 hover:bg-slate-400' };
        ...
        return (
            <div className="relative flex items-center" ref={wrapperRef}>
                {isOpen && (
                    <div className="absolute right-full mr-2 top-1/2 -translate-y-1/2 bg-slate-800 border border-slate-600 rounded-full p-1.5 flex items-center gap-2 shadow-lg z-10">
                        {statusOptions.map(s => 
                            <button 
                                key={s} 
                                onClick={() => { onSelect(s); setIsOpen(false); }} 
                                className={`w-5 h-5 rounded-full transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-blue-500 ${colors[s]}`}
                ...
                <button 
                    ...
                    className={`w-5 h-5 rounded-full shrink-0 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-700 focus:ring-blue-500 ${colors[status]}`} 
    ...
    const statusBorderStyles: { [key in FocusItemStatus]: string } = { green: 'border-green-500', yellow: 'border-yellow-400', red: 'border-red-500', none: 'border-slate-600' };

    return (
        <div className="bg-slate-800 rounded-xl p-6 md:p-8 mt-4 flex-grow flex flex-col gap-6 border border-slate-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-grow min-h-[300px]">
                <div className="flex flex-col">
                    <label className="text-lg font-semibold text-blue-400 mb-2">My Focus</label>
                    <div className="flex-grow w-full bg-slate-700 border border-slate-600 rounded-lg p-3 flex flex-col gap-2">
                        <div className="flex-grow space-y-2 overflow-y-auto pr-2 -mr-3">
                            {focusItems.length === 0 && <p className="text-slate-400 text-center py-4">Click "Add Item" to get started.</p>}
                            {focusItems.map((item) => (
                                <div key={item.id} className={`flex items-start gap-2 group p-2 rounded-md border transition-colors ${statusBorderStyles[item.status]}`}>
                                    ...
                                    <div className="pt-1 flex items-center gap-1.5 shrink-0">
                                        ...
                                        <button onClick={() => handleDeleteItem(item.id)} className="p-1.5 text-slate-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Delete item"><TrashIcon /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="pt-2 mt-auto border-t border-slate-600">
                ...
                <div className="flex flex-col">
                    <label htmlFor="pointers-text" className="text-lg font-semibold text-purple-400 mb-2">Pointers / Notes</label>
                    <textarea id="pointers-text" value={pointersText} onChange={handlePointersChange} className="flex-grow w-full bg-slate-700 border border-slate-600 rounded-lg p-4 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none" aria-label="Pointers and notes" />
                </div>
            </div>
        </div>
    );
