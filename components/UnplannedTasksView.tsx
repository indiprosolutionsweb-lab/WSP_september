...
export const UnplannedTasksView: React.FC<UnplannedTasksViewProps> = ({ unplannedTasks, onAddTask, onUpdateTask, onDeleteTask, onPlanTask, currentWeek }) => {
    ...
    return (
        <div className="bg-slate-800 rounded-xl p-6 md:p-8 mt-4 flex-grow flex flex-col border border-slate-700">
            ...
            <div className="mb-6 bg-slate-700 p-4 rounded-lg shadow-lg">
                 <label htmlFor="new-unplanned-task" className="block text-lg font-semibold text-slate-300 mb-2">Add a new task</label>
                 <div className="flex items-start gap-3">
                    <textarea id="new-unplanned-task" value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} onKeyDown={handleKeyDown} placeholder="Enter task details..." className="flex-grow bg-slate-700 border border-slate-600 rounded-lg p-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y min-h-[40px]" rows={2} />
