...
export const ManagementView: React.FC<ManagementViewProps> = ({ companies, currentUser, onAddCompany, onDeleteCompany, onUpdateUserProfile, onDeleteUser, onCreateUser }) => {
    ...
    return (
        <div className="bg-slate-800 rounded-xl p-6 md:p-8 mt-4 border border-slate-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-700 p-6 rounded-lg shadow-lg">
                    <div className="flex justify-between items-center mb-4 border-b border-slate-600 pb-2">
                        <h3 className="text-xl font-semibold text-blue-400">Users</h3>
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-md mb-6">
                        <h4 className="font-semibold text-slate-300 mb-3">Create New User</h4>
                        <form onSubmit={handleCreateUserSubmit} className="space-y-3">
                            {/* Form fields remain the same */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label htmlFor="new-user-name" className="block text-sm font-medium text-slate-400 mb-1">Full Name</label>
                                    <input id="new-user-name" type="text" value={newUserName} onChange={e => setNewUserName(e.target.value)} required className="w-full bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label htmlFor="new-user-role" className="block text-sm font-medium text-slate-400 mb-1">Role</label>
                                    <select id="new-user-role" value={newUserRole} onChange={e => setNewUserRole(e.target.value as Role.User | Role.Admin)} className="w-full bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500">
                            ...
                            <div>
                                <label htmlFor="new-user-company" className="block text-sm font-medium text-slate-400 mb-1">Company (Optional)</label>
                                <select id="new-user-company" value={newUserCompanyId} onChange={e => setNewUserCompanyId(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    <option value="">Unassigned</option>
                                    {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                            </div>
                            <div>
                                <label htmlFor="new-user-email" className="block text-sm font-medium text-slate-400 mb-1">Email Address</label>
                                <input id="new-user-email" type="email" value={newUserEmail} onChange={e => setNewUserEmail(e.target.value)} required className="w-full bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label htmlFor="new-user-password" className="block text-sm font-medium text-slate-400 mb-1">Password</label>
                                <input id="new-user-password" type="password" value={newUserPassword} onChange={e => setNewUserPassword(e.target.value)} required className="w-full bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                            </div>
                            {creationError && <p className="text-sm text-red-500">{creationError}</p>}
                            <div className="flex justify-end">
                                <button type="submit" disabled={isCreatingUser} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-1.5 rounded-md transition-colors disabled:bg-slate-600 disabled:cursor-wait">
                    ...
                    <div className="mb-4">
                        ...
                        <select id="company-filter" value={selectedCompanyId} onChange={e => setSelectedCompanyId(e.target.value)} className="w-full sm:w-auto bg-slate-900 border border-slate-600 rounded-md px-2 py-1 text-slate-200 focus:outline-none">
                            {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    ...
                    <div className="space-y-3 min-h-[300px]">
                        {viewState.isLoading ? (
                            ...
                            <div key={user.id} className="flex flex-wrap items-center justify-between gap-2 bg-slate-800 p-3 rounded-md border border-slate-600">
                                <div>
                                        ...
                                        {currentUser.role === Role.Superadmin ? (
                                            <div className="relative">
                                                <select
                                                    ...
                                                    className={`text-xs font-bold rounded-full text-white py-0.5 pl-2 pr-6 appearance-none focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-offset-slate-800 focus:ring-blue-500 disabled:opacity-70 disabled:cursor-wait ${user.role === Role.Admin ? 'bg-indigo-500' : 'bg-gray-500'}`}
                                                    aria-label={`Change role for ${user.name}`}
                                                >
                                                    <option value={Role.User} className="bg-slate-700 text-white font-normal">user</option>
                                                    <option value={Role.Admin} className="bg-slate-700 text-white font-normal">admin</option>
                                                </select>
                                ...
                                <div className="flex items-center gap-3">
                                    <select value={user.company_id || ''} onChange={(e) => handleUserCompanyChange(user, e.target.value)} className="bg-slate-900 border border-slate-600 rounded-md px-2 py-1 text-slate-200 text-sm">
                                        <option value="">No Company</option>
                                        {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                    <button onClick={() => openConfirmationModal('user', user.id, user.name)} className="p-1.5 text-slate-500 hover:text-red-500 hover:bg-red-900/30 rounded-full transition-colors" aria-label={`Delete ${user.name}`}>
                                        <TrashIcon />
                                    </button>
                                </div>
                            </div>
                ...
                <div className="bg-slate-700 p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-semibold text-purple-400 mb-4 border-b border-slate-600 pb-2">Companies</h3>
                    <div className="bg-slate-800/50 p-4 rounded-md mb-6">
                        <h4 className="font-semibold text-slate-300 mb-2">Onboard New Company</h4>
                        <div className="flex flex-col sm:flex-row gap-2">
                            <input type="text" placeholder="New Company Name" value={newCompanyName} onChange={(e) => setNewCompanyName(e.target.value)} className="flex-grow bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-purple-500" />
                            <select value={newCompanyCalendar} onChange={e => setNewCompanyCalendar(e.target.value as 'January' | 'April')} className="bg-slate-900 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500">
             ...
             <ConfirmationModal 
                ...
                message={<>Are you sure you want to delete <strong className="text-slate-200">{modalState.name}</strong>? {modalState.type === 'user' && "This will also delete all of their tasks."} This action cannot be undone.</>}
            />
        </div>
    );
};
